...
Review note (2026-03-11): Rechecked after Pass 4 test additions including Playwright responsive layout checks and Tauri runtime E2E drag and drop checks.
...
- `apps/desktop/src/app/shell/WorkspaceApp.pass2.test.tsx`
...
- `apps/desktop/src/services/tauri/core/commands.test.ts`
- `apps/desktop/src/services/runtimeErrorLog.test.ts`
...
Implemented helper: `assertVisibleActionableControls(controls, scopeLabel, options)` in `apps/desktop/src/test/visibleControlAudit.ts`

Current behavior:
- Enumerates visible buttons, inputs, selects, tabs, checkboxes, radios, searchboxes, switches, and textboxes within a scoped container.
- Fails when a visible actionable control in scope has no explicit contract entry.
- Verifies each contracted control is either disabled, produces an observable action, or is documented as an intentional no-op with rationale.
- Includes the scope label and control name in failures so dead controls are easy to triage.

Current usage:
- `apps/desktop/src/app/shell/WorkspaceApp.pass2.test.tsx` uses it to audit visible controls in the About resources card and the Settings maintenance actions.

Remaining expansion:
- Broaden usage from targeted regions to full-workspace sweeps across Library, QC, Playlists, Video Workspace, Settings, About, and Publisher Ops.

### 2. UI Signal Recorder Helper

Implemented helper: `createUiSignalRecorder(options)` in `apps/desktop/src/test/uiSignalRecorder.ts`

Current behavior:
- In Vitest/RTL scenarios, spies on `console.error` and `console.warn` and records visible `role="alert"`, `role="status"`, and `role="dialog"` output.
- Supports allowlists for known-safe alerts, statuses, dialogs, and console noise.
- Produces failure messages that preserve the unexpected signal text for easier triage.

Current usage:
- `apps/desktop/src/app/shell/WorkspaceApp.pass2.test.tsx` wraps the About and search-isolation scenarios with it.

Remaining expansion:
- Add the Playwright-side equivalent for `console`, `pageerror`, `dialog`, and failure screenshots so browser/runtime sweeps get the same signal discipline.
...
| SMK-005 | No fatal startup console/runtime errors | Playwright + Vitest helper | Fail on unexpected console/page errors during first render | Missing | Add the Playwright-side signal recorder to the smoke harness |
...
| UI-004 | Every search field accepts input | Vitest + RTL | Track search updates visible rows and does not mutate unrelated state | Partial | Track search isolation exists; extend the same contract to any additional search/filter surfaces |
| UI-005 | Every visible button is actionable or clearly disabled | Vitest + RTL | Run `assertVisibleActionableControls` per workspace | Partial | Helper exists and is used on About/Settings slices; expand to full-workspace sweeps |
...
| BEH-002 | Search isolation | Vitest + RTL | Typing search only changes search results/pageing; no queue mutation, no playback restart, no codec prep reset, no publish mutation | Partial | Track search isolation assertions exist; extend the same guardrails to other searchable/filterable surfaces |
| BEH-003 | About is informational only | Vitest + RTL | Copy/refresh diagnostics work; no workflow controls leak in; mode-independent | Partial | Dedicated About contract test exists; expand it to cover refresh-after-load and Publish-mode parity end-to-end |
...
| BEH-007 | No dead buttons or fake placeholders | Vitest + RTL | Every enabled visible control yields meaningful effect or explicit rationale | Partial | Audit helper exists and already reproduced dead Settings banner-clear controls; expand beyond targeted slices |
| BEH-008 | About/Settings/workflow labels match actual behavior | Vitest + RTL | Labels like `Reset Library Data`, `Refresh Diagnostics`, `Open Track QC` do what they say and nothing more | Partial | About and Settings label checks now exist; extend to Publish and video-workspace verbs |
| BEH-009 | Runtime error log surface is diagnostic-only | Vitest + RTL + command/service tests | About shows a truthful runtime log path or unavailable state; copied diagnostics include it; runtime errors reach the shared log pipeline | Partial | Command/service tests cover the pipeline; shell test now covers surfaced path and copied diagnostics, but Playwright/runtime verification is still pending |
...
| PLY-006 | Player persistence across workspaces | Vitest + RTL | Shared player remains consistent when moving across Release Preview workspaces; About should preserve it, while Publish may intentionally hide it | Partial | Reproduction test exists and currently fails for About; broaden sweep once the defect is fixed |
...
| NEG-002 | Switching pages during playback | Playwright + Vitest | Playback remains stable, no stale layout or error banners | Partial in Vitest shell coverage | Keep the shell navigation-under-playback scenario and add a Playwright/runtime variant |
...
| NEG-006 | Invalid state restore on restart | Playwright + runtime + Rust integration | Persisted invalid queue/selection/root/render state is pruned or warned on restart | Partial in Vitest shell coverage | Broaden corrupted restart coverage from shell localStorage cases to Playwright/runtime and Rust variants |

## Highest-Value Pass 2 Targets Review

1. Still pending: replace legacy Playwright browser smoke assertions so they target the current shell instead of the old standalone publisher prototype.
2. Partially done: `createUiSignalRecorder` exists for Vitest/RTL, but the Playwright-side signal recorder and screenshot-on-failure flow are still pending.
3. Partially done: `assertVisibleActionableControls` exists and is already catching targeted dead-control regressions, but it has not been rolled out across every workspace yet.
4. Partially done: explicit isolation tests now exist for `Search` and `About`, and Settings dead controls are reproduced; video-preview transport isolation is still pending.
5. Still pending: first-pass responsive Playwright checks for wide, normal, and reduced-width layouts, including Publish mode dock behavior.
