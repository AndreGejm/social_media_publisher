...
import { TauriClientProvider, type TauriClient } from "../services/TauriClientProvider";
...
function renderHookWithProvider<Result, Props>(renderCallback: (initialProps: Props) => Result) {
    return renderHook(renderCallback, {
        wrapper: ({ children }: any) => React.createElement(TauriClientProvider, { client: mockClient as any, children })
    });
}
...
describe("useIngestJobPolling", () => {
    ...
    it("calls onJobsCompleted when a job transitions to COMPLETED", async () => {
        ...
        const setActiveScanJobs = vi.fn((updater: (c: typeof jobs) => typeof jobs) => updater(jobs)) as any;
