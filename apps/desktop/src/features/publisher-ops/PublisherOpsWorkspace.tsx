...
function normalizeAppError(error: unknown): UiAppError {
  const appError = normalizePublisherOpsUiError(error);
  return {
    code: appError.code,
    message: appError.message,
    details: redactErrorDetails(appError.details)
  };
}
...
export default function PublisherOpsWorkspace({
  ...
  const refreshHistory = async () => {
    ...
    try {
      const rows = await listReleaseHistory();
  ...
  const loadReportFor = async (releaseId: string) => {
    ...
    try {
      const result = await getReleaseReport(releaseId);
  ...
  const onLoadSpec = async () => {
    ...
    try {
      const response = await loadReleaseSpec(specPath);
  ...
  const onPlanPreview = async (event: FormEvent) => {
    ...
    try {
      const response = await planRelease({
        spec_path: specPath,
  ...
  const onExecute = async (explicitReleaseId?: string) => {
    ...
    try {
      const response = await executeRelease(releaseId);
