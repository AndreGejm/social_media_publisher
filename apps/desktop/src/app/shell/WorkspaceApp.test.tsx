...
vi.mock("../../features/player/QcPlayer", () => ({
...
describe("WorkspaceApp metadata editor", () => {
  beforeEach(() => {
    vi.spyOn(console, "error").mockImplementation((...args: unknown[]) => {
      const first = String(args[0] ?? "");
      if (first.includes("Not implemented: HTMLMediaElement.prototype.load")) {
        return;
      }
  ...
  it("opens an album-track row context menu and shows the track in Tracks", async () => {
    ...
    const queueCandidateHeadings = await screen.findAllByRole("heading", { name: "Queue Candidate" });
    expect(queueCandidateHeadings.length).toBeGreaterThan(0);
  });
  ...
  it("opens Video Workspace with the Stage 1 static section shell", async () => {
    ...
    expect(within(sections).getByRole("region", { name: "Output" })).toBeInTheDocument();
    expect(within(sections).getByRole("region", { name: "Preview" })).toBeInTheDocument();
    expect(within(sections).getByRole("region", { name: "Render" })).toBeInTheDocument();
  });
  ...
  it("starts in shared output mode by default", async () => {
    ...
    const firstCall = tauriApiMocks.initExclusiveDevice.mock.calls[0];
    expect(firstCall?.[2]).toBe(false);
    expect(screen.getByText(/Output: Shared/)).toBeInTheDocument();
  });

  it("only enters exclusive output mode after explicit user action", async () => {
    let activeMode: "shared" | "exclusive" = "shared";
    tauriApiMocks.initExclusiveDevice.mockImplementation(async (_rate: number, _depth: number, preferExclusive?: boolean) => {
      activeMode = preferExclusive ? "exclusive" : "shared";
      return {
        sample_rate_hz: 48_000,
        bit_depth: 16,
        buffer_size_frames: 256,
        is_exclusive_lock: activeMode === "exclusive"
      };
    });
    tauriApiMocks.getPlaybackContext.mockImplementation(async () => ({
      ...
      output_status: {
        ...
        sample_rate_hz: 48_000,
        bit_depth: 16,
        bit_perfect_eligible: activeMode === "exclusive",
        reasons: ["Exclusive output mode is active."]
      },
      active_queue_index: 0,
      is_queue_ui_expanded: false,
      queued_track_change_requests: 0,
  ...
  it("falls back to shared mode when exclusive acquisition fails", async () => {
    tauriApiMocks.initExclusiveDevice.mockImplementation(async (_rate: number, _depth: number, preferExclusive?: boolean) => {
      if (preferExclusive) {
        throw {
    ...
    await waitFor(() => {
      expect(
        tauriApiMocks.initExclusiveDevice.mock.calls.filter((call) => call[2] === false).length
      ).toBeGreaterThan(1);
    });
    expect(screen.getByText(/Output: Shared/)).toBeInTheDocument();
    expect(screen.getByText(/could not be acquired/i)).toBeInTheDocument();
  });
