...
function Write-UsageError {
    Write-Output "USAGE ERROR: .\scripts\windows\make-exe.ps1 -Mode <release|debug>"
    exit 1
}
...
if (Get-Command "pnpm.cmd" -ErrorAction SilentlyContinue) {
    ...
    "[$timeStamp] FAIL missing tool: pnpm (or corepack)" | Out-File -FilePath $logFile -Append -Encoding utf8
    Write-Output "BUILD FAILED. Tail of log:"
    Get-Content -Path $logFile -Tail 15 | ForEach-Object { Write-Output $_ }
    exit 1
}
...
try {
    ...
    $buildCommand = if ($Mode -eq "release") {
        "$pnpmCommand --filter @release-publisher/desktop tauri build"
    } else {
        "$pnpmCommand --filter @release-publisher/desktop tauri build --debug"
    }
